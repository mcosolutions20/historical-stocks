...
function SearchStock({ userInfo, planInfo, onAuthChanged }) {
  ...
  const {
    ...
    setSortDir,
    handleHeaderSort,
  } = useSorting({ rows: s.rows, mode: s.mode });
  ...
  async function handleUpgrade() {
    await startProDemo();
    onAuthChanged?.();
    await s.submitSearch(); // auto-retry
  }
          ...
          <form onSubmit={handleSubmit} className="row g-3 align-items-end">
            ...
            {s.showTicker && (
              <div className="col-12 col-md-3">
                ...
                <input
                  ...
                  value={s.ticker}
                  onChange={(e) =>
                    s.setTicker(e.target.value.toUpperCase())
                  }
                />
              </div>
            )}
            ...
            {s.showQuantity && (
              <div className="col-12 col-md-3">
                ...
                <input
                  ...
                  value={s.quantity}
                  onChange={(e) =>
                    s.setQuantity(Number(e.target.value))
                  }
                />
              </div>
            )}
            ...
            <div className="col-12 col-md-3">
              ...
              >
                ...
                <DropdownMenu>
                  <DropdownItem onClick={() => s.setPreset("1W")}>
                    Last week
                  </DropdownItem>
                  <DropdownItem onClick={() => s.setPreset("1M")}>
                    Last month
                  </DropdownItem>
                  <DropdownItem onClick={() => s.setPreset("3M")}>
                    Last 3 months
                  </DropdownItem>
                  <DropdownItem onClick={() => s.setPreset("6M")}>
                    Last 6 months
                  </DropdownItem>
                  <DropdownItem onClick={() => s.setPreset("YTD")}>
                    Year to date
                  </DropdownItem>
                  <DropdownItem onClick={() => s.setPreset("1Y")}>
                    Last year
                  </DropdownItem>
                </DropdownMenu>
              </ButtonDropdown>
